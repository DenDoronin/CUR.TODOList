...
{
    DORBubbleView *v = [[self alloc] initWithFrame:CGRectMake(center.x - MARGIN_X, center.y - MARGIN_Y, MARGIN_X*2,
                                                             MARGIN_Y*2)];
...
- (void)layoutSubviews
{
    ...
    if (self.customView) {
     ...
     CGPoint tipPoint = [self convertPoint:self.tipPoint toView:[UIApplication sharedApplication].keyWindow];
   
    if (tipPoint.x > 1) { // position bubble to point to tipPoint
        center.x = self.tipPoint.x;
        if (tipPoint.x + rect.size.width/2 > MAX_WIDTH - MARGIN_X*2)
            center.x -= tipPoint.x + rect.size.width/2 - ( MAX_WIDTH - MARGIN_X*2);
        else
            if (tipPoint.x - rect.size.width/2 < MARGIN_X*2)
...
//        else
//            if (center.x - rect.size.width/2 < MARGIN_X*2)
